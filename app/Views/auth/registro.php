<div style="max-width: 400px; margin: 50px auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); text-align: center;">
    ...
    <?php if (!empty($sucesso)): ?>
        <p style="color: green; font-size: 0.9em; margin-bottom: 15px;"><?= htmlspecialchars($sucesso, ENT_QUOTES, 'UTF-8') ?></p>
    <?php endif; ?>

    <form action="/financas/auth/registro" method="POST" class="d-flex flex-column">
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8') ?>">
        
        <input type="text" name="nome" placeholder="Nome Completo" required autocomplete="name" style="margin-bottom: 15px; padding: 10px;">
        <input type="email" name="email" placeholder="Seu E-mail" required autocomplete="email" style="margin-bottom: 15px; padding: 10px;">
        <input type="password" name="senha" placeholder="Crie uma Senha" required autocomplete="new-password" style="margin-bottom: 20px; padding: 10px;">
